...
fn main() {
    ...
    let comb = Combination::new(500_000, MOD);

    let a = ModInt::new(comb.p(m, n) as u64, MOD as u64);

    let mut b = ModInt::new(0, MOD as u64);

    for i in 0..=n {
        let ptn = ModInt::new(comb.get(n, i) as u64, MOD as u64)
            * ModInt::new(comb.p(m - i, n - i) as u64, MOD as u64);
        if i % 2 == 0 {
            b += ptn;
        } else {
            b -= ptn;
...
impl Combination {
    ...
    pub fn p(&self, x: usize, y: usize) -> usize {
        self.fact[x] * self.inv_fact[x - y] % self.modulo
    }
}
