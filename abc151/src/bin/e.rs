...
fn main() {
    ...
    a.sort();
    let a_modint: Vec<ModInt<u64>> = a
        .iter()
        .map(|&i| {
            let i = i + MOD as i64;
            ModInt::new(i as u64, MOD)
        })
        .collect();

    let comb = Combination::new(n, MOD as usize);
    let mut max_sum = ModInt::new(0, MOD);
    let mut min_sum = ModInt::new(0, MOD);

    for i in 0..n {
        let ai = a_modint[i];
        if i >= k - 1 {
            max_sum += ai * comb.get(i, k - 1) as u64;
        }
        if n - i - 1 >= k - 1 {
            min_sum += ai * comb.get(n - i - 1, k - 1) as u64;
        }
    }

    println!("{}", (max_sum - min_sum).v);
}
